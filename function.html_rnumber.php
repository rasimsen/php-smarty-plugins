...
/**
 * Smarty {html_rnumber} plugin
 *
 * Type: function<br>
 * Name: html_rtext<br>
 * Purpose: sayý alaný için gerekli kontrollere sahip textbox üretir
 *
 * ChangeLog:<br>
 ...
 * @return string
 */
function smarty_function_html_rnumber($params, &$smarty) {
	$debug=0;
	...
	foreach ($params as $_key=>$_value) {
		...
		switch ($_key) {
			...
			case 'not_null':
				...
				$js_arr[standart]='onBlur(this,\'rnumber\')';				
				break;
			case 'onfocus':
				$sonuc[$_key]=$_key.'="'.$_value.';onFocus(this,\'rnumber\')"';
				break;
			case 'onkeypress':
			case 'onkeyup':
			case 'onclick':
	...
	//javascript kontrolü
	$js_arr["rnumber"]='rnumber(this)';
	//stil verilmedi ise
	if(!$sonuc['class'])
		$sonuc['class']='class="rnumber"';
	...
	if($params[mode]=="list" || $params[mode]==3){
 	...
 	if($debug) var_dump($params,$sonuc);
